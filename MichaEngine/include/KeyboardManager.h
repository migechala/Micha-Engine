...
private:
  ...
  std::multimap<SDL_Scancode, std::tuple<std::function<void()>, bool, bool>> listeners; // Registered listeners
...
public:
  ...
  KeyboardManager &operator=(const KeyboardManager &) = delete; // Delete copy assignment operator
  ...
  template <class... Args> bool isNotPressed(Args... keys);
...
template <class... Args> inline bool KeyboardManager::isNotPressed(Args... keys) {

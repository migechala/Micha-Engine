...
std::shared_ptr<SDL_Texture> ResourceLoader::loadTexture(std::shared_ptr<SDL_Renderer> renderer,
  ...
  LOG_INFO("Loaded texture " + location +
               " to address: " + std::to_string(reinterpret_cast<uintptr_t>(texturePtr.get())),
           LOG_LEVEL::PRIORITY);
  std::cout << "Loaded texture " << location << " to address: " << texturePtr << std::endl;
  return texturePtr;
}
